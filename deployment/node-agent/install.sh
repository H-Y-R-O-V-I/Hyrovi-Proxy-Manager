...
systemctl enable --now hyrovi-hpm-node-heartbeat.timer hyrovi-hpm-node-telemetry.timer hyrovi-hpm-node-provisioning.timer
systemctl start hyrovi-hpm-node-heartbeat.service hyrovi-hpm-node-telemetry.service hyrovi-hpm-node-provisioning.service
systemctl --no-pager --full status hyrovi-hpm-node-heartbeat.timer hyrovi-hpm-node-telemetry.timer hyrovi-hpm-node-provisioning.timer
