...
- proxy hosts, including custom proxy locations, automatically inject the same-origin analytics SDK into ordinary HTML responses before `</head>`, while API/binary response bodies are not modified. HYROVI also requests only low-entropy browser Client Hints (UA/mobile/platform) to improve coarse device classification;
...
- multi-node telemetry uses the existing authenticated HYROVI node token: remote nodes push bounded, sanitized security-request batches and pseudonymous browser-activity batches to the controller. The controller stores telemetry per node, keeps remote policies/actions separate from local response state, and exposes a node filter for combined Pi/VPS analysis;
- the reproducible remote-node package lives in `deployment/node-agent/` and contains heartbeat/telemetry uploaders, systemd service/timer units, an environment template, and installer. The uploader uses persistent inode/offset cursors and multiple sub-100-KB catch-up batches so failed uploads do not lose events and the controller can retain its normal JSON request limit;
...
- rule-based Protection behavior matrix for `crawler`, `ddos`, `criticalFiles`, `exploit`, `authAbuse`, `recon` and `unusualMethods`. Each class can independently Observe, Rate-limit IP, Challenge client or Block IP; URI/method classes that can be determined before proxying (`criticalFiles`, `exploit`, `unusualMethods`) can additionally `Deny request`;
- protection behavior inherits Global → Host Group → Proxy Host. A child can override one category while inheriting the remaining categories, and an explicit host policy wins over its group;
- pre-request protection is emitted to per-host `host-protection/<id>.conf` files. Critical file/repository probes such as `.env`, `.git`, `.svn`, `.hg`, `/etc/passwd` and `/proc/self`, selected traversal/injection URI probes, and TRACE/TRACK/CONNECT can therefore return 403/405 before `proxy_pass`; selecting `Block IP` for these classes also lets the normal 5-second monitor block the observed public source after the logged attempt;
...
- host groups that apply shared IP/CIDR access controls, shared security modes and per-attack-class Protection behavior overrides to many Proxy Hosts at once;
...
- `/data/nginx/hyrovi-security/host-protection/*.conf` (generated per-host pre-request deny rules)
