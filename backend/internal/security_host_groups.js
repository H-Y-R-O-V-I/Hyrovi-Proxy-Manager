...
const emptyState = () => ({ version: 1, groups: [] });
...
const writeState = async (state) => {
	...
	await fs.promises.writeFile(tmp, `${JSON.stringify({ version: 1, groups: state.groups }, null, 2)}\n`, "utf8");
	await fs.promises.rename(tmp, GROUPS_FILE);
};

const variableName = (groupId) => `hsg_${createHash("sha256").update(String(groupId)).digest("hex").slice(0, 12)}`;

const buildGeoConfig = (groups) => {
	const lines = ["# Generated by HYROVI Sec host groups. Do not edit manually."];
	for (const group of groups) {
		if (group.accessMode === "open") continue;
		const variable = variableName(group.id);
...
const buildHostAcl = (group) => {
	if (!group || group.accessMode === "open") return "# HYROVI Sec group access: open\n";
	const variable = variableName(group.id);
...
const writeRuntimeFiles = async (groups, hosts) => {
	await fs.promises.mkdir(ACL_DIR, { recursive: true });
	await fs.promises.writeFile(HTTP_GEO_FILE, buildGeoConfig(groups), "utf8");
	const activeIds = new Set();
	for (const host of hosts) {
		activeIds.add(String(host.id));
		await fs.promises.writeFile(`${ACL_DIR}/${host.id}.conf`, buildHostAcl(groupForHost(groups, host.id)), "utf8");
	}
...
const syncNginxForState = async (state) => {
	...
	await writeRuntimeFiles(state.groups, hosts);
...
const applyNextState = async (current, next) => {
	try {
		...
		await writeRuntimeFiles(current.groups, await listHostRows()).catch(() => {});
...
const internalSecurityHostGroups = {
	prepare: async () => {
		...
		await writeRuntimeFiles(state.groups, await listHostRows());
		return state;
	},
	...
	create: async (access, data) => {
		...
		return withMutation(async () => {
			...
			const next = await applyNextState(current, { version: 1, groups: nextGroups });
	...
	delete: async (access, groupId) => {
		...
		return withMutation(async () => {
			...
			await applyNextState(current, { version: 1, groups: current.groups.filter((group) => group.id !== groupId) });
			return { success: true };
		});
	},
	...
	ensureHostAclFile: async (hostId) => {
		...
		await fs.promises.writeFile(`${ACL_DIR}/${hostId}.conf`, buildHostAcl(groupForHost(groups, hostId)), "utf8");
	},
