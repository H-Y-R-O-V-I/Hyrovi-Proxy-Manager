...
touch /data/nginx/hyrovi-security/challenged-ips.geo || true
...
case "${HYROVI_SEC_EMERGENCY_BYPASS,,}" in
	1|true|yes|on)
		...
		printf '%s\n' '# HYROVI Sec emergency bypass active. Stored challenge state is preserved in challenges.json.' > /data/nginx/hyrovi-security/challenged-ips.geo
		;;
esac
